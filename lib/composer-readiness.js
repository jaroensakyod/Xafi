...
export function findReplacementComposer(previousElement, root = document, selectors = INPUT_SELECTORS) {
  const candidate = findVisibleComposerInput(root, selectors);
  if (!candidate || candidate === previousElement) return null;
  return candidate;
}
